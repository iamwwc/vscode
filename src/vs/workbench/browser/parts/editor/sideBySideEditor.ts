...
export class SideBySideEditor extends EditorPane {
	...
	private onConfigurationUpdated(event?: IConfigurationChangeEvent): void {
		if (!event || event.affectsConfiguration('workbench.editor.splitInGroupLayout')) {
			this.orientation = this.configurationService.getValue<'vertical' | 'horizontal'>('workbench.editor.splitInGroupLayout') === 'vertical' ? Orientation.VERTICAL : Orientation.HORIZONTAL;
	...
	private createSplitView(parent: HTMLElement): void {
		...
		// Clear old (if any)
		if (this.splitview) {
			parent.removeChild(this.splitview.el);
